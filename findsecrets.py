...
regex_list = {
    ...
    "Amazon AWS Secret Key": r'awsSecretAccessKey: "[a-zA-Z0-9=:_\+\/-]{5,100}"',
    "Amazon AWS Bucket": r'awsBucket: "[a-zA-Z0-9=:_\+\/-]{5,100}"',
    "Amazon AWS Region": r"us-[a-z]{4}-[0-9]{1}"
}

# "URL Parameter": r"(?<=\?|\&)[a-zA-Z0-9_]+(?=\=)",

def find_secrets(urls, headers):
    ...
    for key, value in regex_list.items():
        ...
        if(match):
            print()
            print(f"{G}[+] {key} encontrado em {i} => {match}{END}")
            info += 1
